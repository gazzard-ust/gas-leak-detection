...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        # DETECTION PARAMETERS - INCREASED FOR BETTER STABILITY
        self.target_lost_frames = 0
        self.detection_confidence_threshold = 0.15  # Lowered for better detection
        self.min_detection_frames = 3  # Need this many consecutive detections to confirm
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Step 3: Object detection with YOLOWorld - LOWER CONFIDENCE THRESHOLD FOR DETECTION
    ...
    def add_status_info(self, frame):
        ...
        if target_object:
            cv2.putText(frame, f"Target: {target_object}", (10, frame.shape[0] - 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        else:
            cv2.putText(frame, "No target set", (10, frame.shape[0] - 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
        # Display current movement command
        cv2.putText(frame, f"Status: {self.movement_command}", (10, frame.shape[0] - 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
...
def set_target(object_name: str):
    ...
    try:
        ...
        stop_cmd = Twist()
        node.movement_pub.publish(stop_cmd)  # Ensure robot is initially stopped
        node.movement_command = "Starting search for " + target_object
        
        node.get_logger().info(f"🚀 New target set: {target_object}")
        return {"message": f"🚀 Tracking: {target_object.capitalize()}"}
...
def root():
    ...
    <html>
    <head>
        <title> Reactive Navigation using YOLOWorld</title>
        <style>
            body {
    ...
    <body>
    <div class="container">
        <h1>Gazzard: Gas Leak Detection and Localization</h1>
        
        <div class="notification" id="targetReachedNotification">
            TARGET REACHED!
        </div>
        
        <div class="controls">
            <input type="text" id="objectInput" placeholder="Enter object name" style="width: 200px;">
            <button onclick="setTarget()">Set Target</button>
            <button onclick="resetTarget()" class="reset-btn">Reset</button>
            <button onclick="stopRobot()" class="stop-btn">EMERGENCY STOP</button>
        </div>
        
        <div class="info">
            <p><strong>Instructions:</strong> Enter an object name (e.g., "bottle", "chair", "cup") and click "Set Target" to start tracking. </p>
        </div>
        
        <h2>Live Feed:</h2>
        <div class="video-container">
            <div class="initial-overlay" id="initialOverlay">Enter an object name above and click "Set Target" to begin tracking</div>
            <div class="video-overlay" id="targetReachedOverlay">TARGET REACHED!</div>
            <div class="video-overlay" id="emergencyOverlay" style="background-color: rgba(255, 0, 0, 0.7);">⚠️ EMERGENCY STOP ⚠️</div>
            <img src="/video_feed" width="1280" alt="Robot camera feed">
        </div>
        
        <!-- Robot Status section completely removed -->
    </div>
    ...
    <script>
        ...
        // Check robot status regularly - simplified version
        function checkRobotStatus() {
            fetch('/robot_status')
                .then(response => response.json())
                .then(data => {
                    // Only check for emergency states and overlays
                    ...
                    // Check if a target has been set (hide initial overlay once target is set)
                    if (data.command !== "Waiting for target" && data.command !== "Waiting for target to be set") {
        ...
        // Check for target reached status regularly
            ...
            fetch('/target_status')
                ...
                .then(data => {
                    if (data.notification && data.notification.active) {
                        // Show notification
                        document.getElementById('targetReachedNotification').style.display = 'block';
                        document.getElementById('targetReachedOverlay').style.display = 'block';
                        
                        // Update the notification text with distance
                        document.getElementById('targetReachedNotification').innerText = 
                            `TARGET REACHED! (${data.notification.distance.toFixed(1)}cm)`;
                        document.getElementById('targetReachedOverlay').innerText = 
                            `TARGET REACHED! (${data.notification.distance.toFixed(1)}cm)`;
                    } else {
                        // Hide notification if not active
                        document.getElementById('targetReachedNotification').style.display = 'none';
                        
                        // Only hide overlay if we're not in emergency stop state either
                        if (!document.getElementById('emergencyOverlay').style.display === 'block') {
                            document.getElementById('targetReachedOverlay').style.display = 'none';
                        }
                    }
                })
                .catch(error => console.error('Error checking target status:', error));
        }
        
        function setTarget() {
            let obj = document.getElementById("objectInput").value;
            if (!obj.trim()) {
            ...
            fetch(`/set_target?object_name=${obj}`)
                .then(response => response.json())
                .then(data => {
                    alert(data.message);
                });
        ...
        function stopRobot() {
            fetch('/stop')
                ...
                .then(data => {
                    ...
                    document.getElementById('emergencyOverlay').style.display = 'block';
                    // Hide target reached overlay if it's showing
                    document.getElementById('targetReachedOverlay').style.display = 'none';
                });
        }
        ...
        // Check target status every second
        setInterval(checkTargetStatus, 1000);
    </script>
    </body>
    </html>
    """)
